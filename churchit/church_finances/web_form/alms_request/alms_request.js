frappe.ready(function () {
	...
	if (recipient_ctrl && type_ctrl) {
		...
		var real_name =
			(frappe.web_form.doc && frappe.web_form.doc.recipient) || "";
		...
		function search(term) {
			...
			timer = setTimeout(function () {
				frappe.call({
					method: "churchit.church_website.api.search_church_recipient",
		...
		if (real_name && frappe.web_form.doc.recipient_type) {
			frappe.call({
				method: "churchit.church_website.api.search_church_recipient",
				args: {
					doctype: frappe.web_form.doc.recipient_type,
					txt: real_name,
				},
				callback: function (r) {
					var results = r.message || [];
					for (var i = 0; i < results.length; i++) {
						if (
							results[i].name === real_name ||
							results[i].label === real_name
						) {
		...
		if (!frappe.web_form.doc.name) {
			frappe.call({
				...
				callback: function (r) {
					...
					var requestor_ctrl =
						frappe.web_form.fields_dict["requestor"];
					...
					Promise.resolve(type_ctrl.set_value("Person")).then(
						function () {
