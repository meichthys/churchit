...
frappe.ui.form.on("Function Sign-Up", {
	refresh(frm) {
		...
		frm.set_query("item", "table_iprj", function (doc) {
			...
			return {
				query: "church.church_ministries.doctype.function_sign_up.function_sign_up.get_function_items",
...
frappe.ui.form.on("Function Sign-Up Item", {
	item(frm, cdt, cdn) {
		...
		frappe.call({
			method:
				"church.church_ministries.doctype.function_sign_up.function_sign_up.get_item_status",
			args: {
